...
@kwdef struct param_struct
    ...
    model_name::String = ""
    save_name::String = "test_Pt1_xdim10"
    changelog::String = ""
    arg_num = 0
    ...
    max_iter_em::Int64 = 1e5
    test_iter::Int64 = 50
    early_stop = true

    pt_list::UnitRange{Int64} = 1:1

    x_dim_fast::Array{Int64,1} = round.(Int64, 10:5:60)
    x_dim_slow::Array{Int64,1} = round.(Int64, 50:5:80)
    do_fast = true
    ...
    ssid_ICA::Bool = false
    ssid_new_init::Bool = false

    y_transform::String = "PCA"
    PCA_ratio::Float64 = 0.99
    PCA_maxdim::Int64 = 1000
    y_baseline::Bool = false
    y_ICA::Bool = false
    remove_ERP::Bool = false

    do_trial_sel::Bool = false # select trials
    decimate = false # keep every other sample
    ...
    mu_Q::UniformScaling{Float64} = .01I
    mu_R::UniformScaling{Float64} = .001I
    mu_P0::UniformScaling{Float64} = .1I
...
@kwdef struct data_struct
    ...
    epoch_sel::Vector{Int64} = zeros(0);
    ...
    pca_R2::Float64 = 0.0;
    pca_C::Matrix{Float64} = zeros(0,0);
...
function transform_model_orth(mdl::model_struct, W)::model_struct
